...
import { setCredentials, logout as logoutAction } from '../../store/slices/NFTAuthSlice';
import axios from 'axios';

let globalLoginSemaphore = false;

export const useNFTAuth = () => {
  const dispatch = useDispatch();
  const { account, signMessage } = useWeb3();
  const [isLoading, setIsLoading] = useState(false);
  const [isLoginInProgress, setIsLoginInProgress] = useState(false);
  
  const { isAuthenticated, token, user } = useSelector((state) => state.nftAuth);

  useEffect(() => {
    if (!token) return;
    const tokenExpiration = localStorage.getItem('nftAuthTokenExpiration');
  ...
  const verifyToken = async () => {
    try {
      ...
      console.error('Error verifying NFT token:', error);
      logout();
    }
  };
  ...
  const login = async () => {
    ...
    if (globalLoginSemaphore) {
      console.log('🔒 Глобальний семафор: NFT логін вже в процесі, пропускаємо');
      return;
    }
  
    if (isLoginInProgress) {
      console.log('🔒 Локальний захист: NFT логін вже в процесі, пропускаємо повторний виклик');
      return;
    }

    if (isAuthenticated && token) {
      console.log('✅ Користувач вже авторизований у NFT marketplace, пропускаємо логін');
      return user;
    }

    console.log('🚀 Починаємо процес NFT логіну для:', account);
    ...
    try {
      ...
      if (verifyResponse.data.isSuccess) {
        ...
        localStorage.setItem('nftAuthTokenExpiration', expirationTime.toString());
        
        localStorage.setItem('nftAuthWalletAddress', account);
        
        console.log('✅ NFT авторизація успішна для:', account);
        return userData;
      } else {
  ...
  const logout = () => {
    ...
    setIsLoginInProgress(false);
    localStorage.removeItem('nftAuthTokenExpiration');
    localStorage.removeItem('nftAuthWalletAddress');
    console.log('🔓 NFT вихід: всі семафори скинуті');
  };
  ...
  useEffect(() => {
    const handleWalletConnected = async (event) => {
      ...
      console.log('🎯 NFT подія walletConnected отримана для:', walletAddress);
      
      if (!walletAddress || walletAddress !== account) {
        console.log('❌ NFT адреса не співпадає з поточним account');
        return;
      }
      
      if (globalLoginSemaphore) {
        console.log('🔒 NFT глобальний семафор активний, пропускаємо автологін');
        return;
      }
      
      if (isAuthenticated || token) {
        console.log('✅ NFT користувач вже авторизований, пропускаємо автологін');
        return;
      }

      if (isLoginInProgress) {
        console.log('🔒 NFT локальний логін вже в процесі, пропускаємо');
        return;
      }

      setTimeout(async () => {
        if (globalLoginSemaphore) {
          console.log('🔒 NFT семафор активний під час setTimeout, пропускаємо автологін');
          return;
        }
        
        try {
          console.log('🚀 Запускаємо автоматичний NFT логін для:', walletAddress);
          await login();
        } catch (error) {
          console.log('❌ NFT Auto-login failed, user can login manually:', error.message);
        }
      }, 500);
    };
