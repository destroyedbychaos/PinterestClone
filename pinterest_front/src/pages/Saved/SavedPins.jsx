  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE}/pins/saved`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        if (!res.ok) throw new Error('Не вдалося завантажити збережені піни');
        const data = await res.json();
        const list = (data?.Pins || data?.pins || []).map(p => ({
          id: p.Id || p.id,
          image: p.ImageUrl || p.imageUrl || p.image,
          title: p.Title || p.title,
          description: p.Description || p.description,
          author: p.UserName || p.userName || p.author,
          tags: (p.Tags || p.tags || '')
            .split(',')
            .map(t => t.trim())
            .filter(Boolean),
